...
export function SectionHeading({
  ...
  return (
    <div className="space-y-2">
      ...
      {description ? <p className="max-w-3xl text-sm leading-6 theme-muted">{description}</p> : null}
    </div>
  );
}
  ...
  return (
    <motion.div whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 280, damping: 24 }} className="surface p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] theme-subtle">{title}</p>
          <strong className="mt-3 block text-2xl font-semibold theme-text">{value}</strong>
        </div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center dark:border-white/10 dark:bg-white/5">
      ...
      <p className="mt-2 max-w-sm text-sm theme-muted">{description}</p>
    </div>
  );
}

export function Badge({ children }: { children: ReactNode }) {
  return <span className="theme-chip">{children}</span>;
}
